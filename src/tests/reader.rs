...
fn seek_data() {
    ...
    let mut seek = r.read_seek_data().unwrap().into_iter();

    let entry = seek.next().unwrap();

    assert_eq!(entry.id, vec![0x15, 0x49, 0xa9, 0x66]);
    assert_eq!(entry.position, 64);

    let entry = seek.next().unwrap();
    assert_eq!(entry.id, vec![0x16, 0x54, 0xae, 0x6b]);
    assert_eq!(entry.position, 172);

    let entry = seek.next().unwrap();
    assert_eq!(entry.id, vec![0x11, 0x4d, 0x9b, 0x74]);
    assert_eq!(entry.position, 2165107);

    let entry = seek.next().unwrap();
    assert_eq!(entry.id, vec![0x1c, 0x53, 0xbb, 0x6b]);
    assert_eq!(entry.position, 3393);
}
...
fn segment_info() {
    ...
    let uid = vec![
        0x46, 0xc6, 0x9d, 0x45, 0xa1, 0x85, 0xa9, 0x29, 0x4d, 0x3d, 0x0a, 0x2f, 0x75, 0x00, 0x56, 0xbd,
    ];
